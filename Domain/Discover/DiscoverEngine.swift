...
enum DiscoverEventKind: String, Codable, Sendable {
    case open, dwell, play, watchlist, watched, voteUp, voteDown
}

struct DiscoverEvent: Codable, Equatable, Sendable {
    var kind: DiscoverEventKind
    var metaId: String
    var metaType: String
    var timestamp: Date

    init(kind: DiscoverEventKind, metaId: String, metaType: String, timestamp: Date = Date()) {
        self.kind = kind
        self.metaId = metaId
...
struct ProfileSnapshot: Codable, Equatable, Sendable {
    var cast: [Int]      // TMDB person ids (top 5)
    var directors: [Int]
    var creators: [Int]
    var genres: [String]
    var keywords: [Int]  // TMDB keyword ids
    var decade: Int?
    var language: String?
...
struct DiscoverAffinity: Codable, Equatable, Sendable {
    var cast: [Int: Double]
    var directors: [Int: Double]
    var creators: [Int: Double]
    var genres: [String: Double]
    var keywords: [Int: Double]
...
enum DiscoverEngine {
    ...
    static let kindWeights: [DiscoverEventKind: Double] = [
    ...
    static func recencyWeight(timestamp: Date, now: Date = Date()) -> Double {
    ...
    static func score(candidate: ProfileSnapshot, affinity: DiscoverAffinity) -> Double {
        var total = 0.0
...
actor DiscoverEventStore {

    static let shared = DiscoverEventStore()
    ...
    func record(_ event: DiscoverEvent) async {
